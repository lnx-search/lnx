...
impl TypeCast {
    ...
    pub fn try_cast_value<'a>(&self, value: Value<'a>) -> Result<Value<'a>> {
        match value {
    ...
    fn err_invalid_value(&self, value: impl UserDisplayType + Debug) -> anyhow::Error {
        anyhow!(
    ...
    pub fn try_cast_str<'a>(&self, string: &'a str) -> Result<Value<'a>> {
        self.try_cast_cow(Cow::Borrowed(string))
    }

    /// Attempts to cast a string to the cast type.
    pub fn try_cast_cow<'a>(&self, string: Cow<'a, str>) -> Result<Value<'a>> {
        match self {
            ...
            Self::DateTime(parser) => {
                parser.try_parse_str(string.as_ref()).map(Value::DateTime)
            },
            Self::IpAddr => {
            ...
            TypeCast::Facet => {
                if let Ok(facet) = Facet::from_text(&string) {
                    ...
                    Err(self.err_with_detail(
                        string,
                        "because the value is not in the form of a path",
                    ))
                }
            },
        }
    ...
    pub fn try_cast_u64<'a>(&self, v: u64) -> Result<Value<'a>> {
        match self {
    ...
    pub fn try_cast_i64<'a>(&self, v: i64) -> Result<Value<'a>> {
        match self {
    ...
    pub fn try_cast_f64<'a>(&self, v: f64) -> Result<Value<'a>> {
        match self {
    ...
    pub fn try_cast_bool<'a>(&self, v: bool) -> Result<Value<'a>> {
        match self {
    ...
    pub fn try_cast_datetime<'a>(&self, v: DateTime) -> Result<Value<'a>> {
        match self {
    ...
    pub fn try_cast_ip<'a>(&self, v: Ipv6Addr) -> Result<Value<'a>> {
        match self {
    ...
    pub fn try_cast_bytes<'a>(&self, v: Vec<u8>) -> Result<Value<'a>> {
        match self {
...
mod tests {
    ...
    fn test_cast_str() {
        ...
        assert_eq!(
            value.unwrap(),
            Value::Bytes(vec![104u8, 101, 108, 108, 111, 32, 119, 111, 114, 108, 100])
        );
        ...
        let value =
            TypeCast::IpAddr.try_cast_str("2345:0425:2CA1:0000:0000:0567:5673:23b5");
        assert_eq!(
            value.unwrap(),
            Value::IpAddr(
    ...
    fn test_cast_u64() {
        ...
        assert_eq!(value.unwrap_err().to_string(), "Cannot cast `u64` to `f64`");
        ...
        assert_eq!(value.unwrap_err().to_string(), "Cannot cast `u64` to `ip`");
    ...
    fn test_cast_i64() {
        ...
        assert_eq!(value.unwrap_err().to_string(), "Cannot cast `i64` to `f64`");
        ...
        assert_eq!(value.unwrap_err().to_string(), "Cannot cast `i64` to `ip`");
    ...
    fn test_cast_f64() {
        ...
        assert_eq!(value.unwrap_err().to_string(), "Cannot cast `f64` to `u64`");
        let value = TypeCast::I64.try_cast_f64(0.0);
        assert_eq!(value.unwrap_err().to_string(), "Cannot cast `f64` to `i64`");
        ...
        assert_eq!(value.unwrap_err().to_string(), "Cannot cast `f64` to `ip`");
    ...
    fn test_cast_bool() {
        ...
        assert_eq!(value.unwrap_err().to_string(), "Cannot cast `bool` to `ip`");
        let value = TypeCast::DateTime(DateTimeParser::default()).try_cast_bool(false);
        assert_eq!(
            value.unwrap_err().to_string(),
    ...
    fn test_cast_ip() {
        ...
        assert_eq!(value.unwrap(), Value::from("2345:425:2ca1::567:5673:23b5"));
        ...
        assert_eq!(value.unwrap_err().to_string(), "Cannot cast `ip` to `u64`");
        let value = TypeCast::I64.try_cast_ip(ipv4);
        assert_eq!(value.unwrap_err().to_string(), "Cannot cast `ip` to `i64`");
        ...
        assert_eq!(value.unwrap_err().to_string(), "Cannot cast `ip` to `bool`");
    }
    ...
    fn test_cast_datetime() {
        ...
        let value =
            TypeCast::DateTime(DateTimeParser::default()).try_cast_datetime(max_time);
        assert_eq!(value.unwrap(), Value::from(max_time));
        let value =
            TypeCast::DateTime(DateTimeParser::default()).try_cast_datetime(min_time);
        assert_eq!(value.unwrap(), Value::from(min_time));
        let value =
            TypeCast::DateTime(DateTimeParser::default()).try_cast_datetime(random_time);
