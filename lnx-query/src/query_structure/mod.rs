...
pub use base::{AsQuery, AsQueryTerm, BuildQueryError, InvalidTermValue};
...
use self::term_value::TermValue;

#[derive(Debug, Clone, Validate, Deserialize, ToSchema)]
#[validate(schema(
    function = "validators::validate_ops",
    arg = "&'v_a mut validators::ValidationConfig"
))]
    ...
    pub fn validate_ops(
        ...
        if let Some(inner_layer) = layer.pipeline.as_ref() {
            ...
            if layers.len() > cfg.max_pipeline_queries {
                ...
                error.add_param(
                    Cow::Borrowed("max_sub_queries_per_pipeline"),
                    &cfg.max_pipeline_queries,
                );
                error.add_param(
                    Cow::Borrowed("current_sub_queries_per_pipeline"),
                    &layers.len(),
                );
                error.add_param(
                    Cow::Borrowed("max_sub_queries_total"),
                    &cfg.max_sub_queries_total,
                );
                error.add_param(
                    Cow::Borrowed("current_sub_queries_total"),
                    &cfg.sub_queries_counter,
                );
                return Err(error);
            }
