...
pub fn validate_select_fields(
    ...
    if fields.is_empty() {
        let message = "at least one field or wildcard must be provided to the `$select` field".to_string();
        let help = "you can use a wildcard to select all fields: `$select: [\"*\"]` ".to_string();        
        let error = context.build_error(ErrorCode::MissingSelectFields, message, Some(help));
        return Err(error)        
    }
    ...
    for (idx, field_name) in fields.iter().enumerate() {
        ...
        let field = schema.get_field(field_name)
            .map_err(|_| make_field_does_not_exist_error(context, table_name, field_name, fields))?;
...
fn make_field_does_not_exist_error(
    ...
    let message = format!("the field {field_name:?} does not exist within table {table_name:?}");
...
fn make_field_not_stored_error(
    ...
    let help = "fields are always stored by default unless explicitly disabled, to re-enable \
     storing after it has been disabled requires deleting and re-creating the table".to_string();
    let message = format!("the field {field_name:?} within table {table_name:?} is not stored");
