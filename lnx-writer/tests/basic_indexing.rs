...
async fn test_basic_indexing_flow() {
    ...
    assert_eq!(
        writer.stats().segments_produced(),
        2,
        "Two segments should exist after the auto commit period has elapsed."
    );
    assert_eq!(
        writer.stats().documents_ingested(),
        2,
        "Two documents should have been ingested."
    );
}
...
async fn test_basic_delete_flow() {
    ...
    assert_eq!(
        writer.stats().deletes_registered(),
        2,
        "Deletes should have been registered."
    );
}
