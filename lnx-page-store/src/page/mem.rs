...
impl PageEncodeBuffer {
    ...
    pub fn write_bytes(&mut self, bytes: &[u8]) {
        assert!(self.initialised_len + bytes.len() <= self.buffer.len(), "Page encoding tried to write out of bounds");
    ...
    pub unsafe fn advance_initialised_cursor(&mut self, by: usize) {
        assert!(self.initialised_len + by <= self.buffer.len(), "Cursor would go out of bounds");
