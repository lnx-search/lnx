...
fn test_encode_decode(#[case] metadata: SampleMetadata, #[case] encrypt: bool) {
    ...
    let loaded_metadata =
        decode_page_file_metadata::<SampleMetadata>(cipher.as_ref(), &mut buffer)
...
fn test_decode_err_missing_magic_bytes(#[case] encryption: Encryption) {
    ...
    let err = decode_page_file_metadata::<SampleMetadata>(cipher.as_ref(), &mut buffer)
        .unwrap_err();
...
fn test_decode_err_missing_encryption_hint(
    ...
    let err = decode_page_file_metadata::<SampleMetadata>(
        cipher.as_ref(),
        &mut buffer[..slice_at],
    )
    .unwrap_err();
...
fn test_decode_err_missing_context() {
    ...
    let err =
        decode_page_file_metadata::<SampleMetadata>(cipher.as_ref(), &mut buffer[..20])
            .unwrap_err();
    assert_eq!(
        err.to_string(),
...
fn test_decode_err_missing_decryption_cipher() {
    ...
    let err =
        decode_page_file_metadata::<SampleMetadata>(None, &mut buffer).unwrap_err();
    assert_eq!(
        err.to_string(),
...
fn test_decode_err_encryption_fail_key_wrong() {
    ...
    let err = decode_page_file_metadata::<SampleMetadata>(Some(&cipher), &mut buffer)
        .unwrap_err();
    assert_eq!(err.to_string(), DecodeError::DecryptionFailed.to_string());
}
