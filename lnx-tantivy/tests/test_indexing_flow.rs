...
async fn test_full_indexing_flow() {
    ...
    let results = tokio::task::spawn_blocking(move || {
        ...
        searcher.search(&query, &TopDocs::with_limit(10)).expect("Search index")
    }).await.unwrap();

    assert_eq!(results.len(), 1);
    let searcher = reader.searcher();
    let doc: tantivy::TantivyDocument = searcher.doc_async(results[0].1)
        .await
