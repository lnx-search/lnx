...
impl ExecutorPool {
    ...
    pub fn create(name: impl Into<String>, pool_size: usize) -> Result<Self, CreatePoolError> {
        let name = name.into();
...
mod tests {
    ...
    async fn test_create_and_use_pool() {
        let pool = ExecutorPool::create("test_1", 1)
            .expect("Create pool Ok");
        let result = pool.run(move || {
            "hello"
        }).await.expect("Tasks should not be cancelled.");
        assert_eq!(result, "hello");
    }
    
    #[tokio::test]
    async fn test_create_empty_pool_err() {
        let _err = ExecutorPool::create("test_1", 0)
            .expect_err("Create pool should error");
    }
    ...
    async fn test_pool_panic() {
        ...
        pool.run(move || {
            panic!("demo!")
        }).await.expect_err("Task panic should result in error");
    }
