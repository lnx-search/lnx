...
impl FilePageCache {
    ...
    #[inline]
    /// The _approximate_ number of pages allocated.
    pub fn num_allocated_pages(&self) -> usize {
        self.primary.entry_count() as usize
    }

    #[inline]
    /// The _approximate_ number of allocated bytes in the cache.
    pub fn num_allocated_bytes(&self) -> usize {
        self.primary.entry_count() as usize * self.page_size.num_bytes()
    }
    ...
    pub fn prepare_read<K: Hash>(
        ...
        self.prepare_read_inner(hasher.finish(), hasher, range)
    }
    ...
    pub fn delete_cache_entry<K: Hash>(&self, file: K) {
        let mut hasher = self.random_state.build_hasher();
        file.hash(&mut hasher);

        self.delete_cache_entry_inner(hasher.finish(), hasher)
    }
...
impl FileBlockState {
    fn allocate(file_id: u64, file_len: usize, page_size: PageSize) -> io::Result<Self> {
    ...
    fn mark_for_deletion(&self, page_id: PageId) {
        ...
        let cleanup_callback = move || {
            let callback_generation = generation_id;
            ...
            unsafe {
                // Try to acquire the lock, if we cannot, this means someone els.
                let Some(_guard) = page_state.try_acquire_write_guard() else {
                    return false;
                };
                ...
                // Check if the page no longer wants to freed due to a new
                // operation overriding this operation.
                ...
                if did_free {
                    ...
                    page_state.mark_free_unchecked();
                }
...
mod tests {
    ...
    fn test_small_cache_create() {
        ...
        cache
            .prepare_read("test.txt", 0..8 << 10)
            .expect_err("Cache entry should not exist");

        cache
            .create_cache_entry("test.txt", 64 << 10)
            .expect("Create entry");
        ...
        assert!(read.next_outstanding_write().is_none());

        let write_request = outstanding_write.unwrap();
        assert_eq!(write_request.read_bytes_range(), 0..8 << 10);

        // Populate and complete the write
        write_request.write(&[1; 8 << 10]);

        let read_ref =
            block_on(read.try_finish()).expect("All writes should be completed");
        assert_eq!(read_ref.as_ref(), &[1; 8 << 10]);
    }

    #[test]
    fn test_unaligned_size_file() {
        let cache = FilePageCache::new(1 << 30, PageSize::Size8KB);
        ...
        cache
            .create_cache_entry("test.txt", 15)
            .expect("Create entry");
        ...
        let write_request = outstanding_write.unwrap();
        assert_eq!(write_request.read_bytes_range(), 0..15);

        // Populate and complete the write
        write_request.write(&[1; 15]);

        let read_ref =
            block_on(read.try_finish()).expect("All writes should be completed");
        assert_eq!(read_ref.as_ref(), &[1; 15]);
    }

    #[test]
    fn test_basic_page_eviction_scheduling() {
        let cache = FilePageCache::new(8 << 10, PageSize::Size8KB);

        // Even though the cache is 8KB in size, the one file with 15 bytes will use
        // up the whole space in the cache as the page size is set to 8KB.
        cache
