...
async fn test_read_cache_used() {
    ...
    let vfs = VirtualFileSystem::mount(dir.path().to_path_buf(), rt_options).await.unwrap();
    
    let bucket = vfs.create_bucket("test")
        .await
        .expect("Create bucket")
        .enable_statistics_return();
    
    let result = bucket
        .write("example.txt", Body::complete(Bytes::from_static(b"Hello, world!")))
        .await
        .expect("Write file");
